...
function SplashCursor({
    ...
    useEffect(() => {
        ...
        function getContext(canvas) {
            ...
            function supportFormat(gl, internalFormat, format, type) {
            ...
            function getFormat(gl, internalFormat, format, type) {
                if (!supportFormat(gl, internalFormat, format, type)) {
                    switch (internalFormat) {
                        case gl.R16F:
                            return getFormat(gl, gl.RG16F, gl.RG, type);
                        case gl.RG16F:
                            return getFormat(gl, gl.RGBA16F, gl.RGBA, type);
                        default:
                            return null;
                    }
                }
            ...
            if (isWebGL2) {
                formatRGBA = getFormat(gl, gl.RGBA16F, gl.RGBA, halfFloatTexType);
                formatRG = getFormat(gl, gl.RG16F, gl.RG, halfFloatTexType);
                formatR = getFormat(gl, gl.R16F, gl.RED, halfFloatTexType);
            } else {
                formatRGBA = getFormat(gl, gl.RGBA, gl.RGBA, halfFloatTexType);
                formatRG = getFormat(gl, gl.RGBA, gl.RGBA, halfFloatTexType);
                formatR = getFormat(gl, gl.RGBA, gl.RGBA, halfFloatTexType);
            }
    ...
    `;
        ...
        function resizeDoubleFBO(target, w, h, internalFormat, format, type, param) {
            ...
            let newRead = createFBO(w, h, internalFormat, format, type, param);
            copyProgram.bind();
    ...
    }, [SIM_RESOLUTION, DYE_RESOLUTION, DENSITY_DISSIPATION, VELOCITY_DISSIPATION, PRESSURE, CURL, SPLAT_RADIUS, SPLAT_FORCE, COLOR_UPDATE_SPEED, SHADING]);
